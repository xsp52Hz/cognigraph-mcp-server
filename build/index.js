...
// --- Type guards ---
const isValidGenerateArgs = (args) => typeof args === 'object' && args !== null && typeof args.markdown === 'string' &&
    (args.outputFormat === undefined || ['html', 'svg'].includes(args.outputFormat));
const isValidGenerateAndSaveArgs = (args) => typeof args === 'object' && args !== null && typeof args.markdown === 'string' &&
    (args.outputFormat === undefined || ['html', 'svg'].includes(args.outputFormat)) &&
    (args.outputDir === undefined || typeof args.outputDir === 'string') &&
    (args.outputFilename === undefined || typeof args.outputFilename === 'string');
const isValidGraphArgs = (args) => typeof args === 'object' && args !== null && typeof args.mermaid_text === 'string' &&
    (args.outputFormat === undefined || ['svg', 'png'].includes(args.outputFormat)) &&
    (args.outputDir === undefined || typeof args.outputDir === 'string') &&
    (args.outputFilename === undefined || typeof args.outputFilename === 'string');
const isValidKnowledgeGraphArgs = (args) => typeof args === 'object' && args !== null && typeof args.markdown === 'string' &&
    (args.outputFormat === undefined || ['svg', 'png'].includes(args.outputFormat)) &&
    (args.outputDir === undefined || typeof args.outputDir === 'string') &&
    (args.outputFilename === undefined || typeof args.outputFilename === 'string') &&
...
class MindmapServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            const determineSavePath = (dirArg, defaultFilenamePrefix, format, filenameArg) => {
                let targetDir;
                if (dirArg) {
            ...
            if (toolName === 'generate_mindmap') {
                if (!isValidGenerateArgs(args))
                    throw new McpError(ErrorCode.InvalidParams, 'Invalid args for generate_mindmap.');
                ...
                try {
                    ...
                    let msg = 'Error generating mind map';
                    error.stderr && (msg += `: ${error.stderr}`);
                ...
                if (!isValidGenerateAndSaveArgs(args))
                    throw new McpError(ErrorCode.InvalidParams, 'Invalid args for generate_and_save_mindmap.');
                ...
                try {
                    ...
                    let msg = `Error saving mind map to ${finalOutputPath}`;
                    error.stderr && (msg += `: ${error.stderr}`);
                    error.message && (msg += `: ${error.message}`);
                    console.error(msg, error);
                    return { content: [{ type: 'text', text: msg }], isError: true };
                ...
                if (!isValidGraphArgs(args))
                    throw new McpError(ErrorCode.InvalidParams, 'Invalid args for generate_relationship_graph.');
                ...
                try {
                    ...
                    let msg = `Error saving relationship graph to ${finalOutputPath}`;
                    error.stderr && (msg += `: ${error.stderr}`);
                    error.message && (msg += `: ${error.message}`);
                    console.error(msg, error);
                    return { content: [{ type: 'text', text: msg }], isError: true };
                    ...
                    try {
                        await unlink(tempMmdFile);
                    }
                    catch { }
                }
            }
            else if (toolName === 'generate_knowledge_graph') {
                if (!isValidKnowledgeGraphArgs(args))
                    throw new McpError(ErrorCode.InvalidParams, 'Invalid args for generate_knowledge_graph.');
...
3.  Ensure the output contains ONLY the Mermaid code block (starting with \`\`\`mermaid and ending with \`\`\`) and absolutely nothing else before or after it.`;
                try {
                    ...
                    try {
                        ...
                        let msg = `Error rendering/saving generated knowledge graph to ${finalOutputPath}`;
                        renderError.stderr && (msg += `: ${renderError.stderr}`);
                        renderError.message && (msg += `: ${renderError.message}`);
                        console.error(msg, renderError);
                    ...
                    let msg = `Error generating knowledge graph using AI model ${model}`;
                    error.response && (msg += `: ${JSON.stringify(error.response.data)}`);
                    error.message && (msg += `: ${error.message}`);
                    console.error(msg, error);
                    return { content: [{ type: 'text', text: msg }], isError: true };
                }
